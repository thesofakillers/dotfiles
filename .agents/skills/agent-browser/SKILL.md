...
Auto-save and restore cookies plus `localStorage`.
...
Credentials are stored encrypted and reused by name.
...
Lightpanda does not support `--extension`, `--profile`, `--state`, or `--allow-file-access`. Install Lightpanda from the [Lightpanda installation guide](https://lightpanda.io/docs/open-source/installation).
